...
const NewAuction = () => {
  ...
  function saveCoordinate(value: string, index: number) {
    ...
    const joined = coordinates.join("|");
        ...
        await fetchImageAndConvertToBase64(`https://maps.googleapis.com/maps/api/staticmap?size=400x400&maptype=satellite&path=color:0xff0000ff|weight:0|${joined}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`)
    ...
    function changePage(e: any, type: string) {
        ...
        if (type == "next") {
            ...
            if(object) {
                if(object.satelliteImage == "" || object.satelliteImage == undefined || object.satelliteImage == null || object.base64Image == undefined || object.base64Image == null || object.base64Image == "") {
            ...
            <div className="w-full mt-24 flex flex-row items-center justify-center gap-2">
                {
                    isFirstPage && (
                        <>
                            ...
                            <button onClick={(e) => {changePage(e, "next")}}  className="w-[140px] h-[40px] text-white font-bold flex justify-center items-center rounded-xl bg-blue-500">
                                Next Page
                            </button>
                        </>
                    )
...

                {
                    isSecondPage && (
                        <>
                            <button onClick={(e) => {changePage(e, "prev")}} className="w-[140px] h-[40px] text-white font-bold flex justify-center items-center rounded-xl bg-blue-500">
                                Back Page
                            </button>
                            <button disabled className="w-[140px] h-[40px] text-white font-bold flex justify-center items-center rounded-xl bg-blue-500 disabled:opacity-50">
                                Next Page
                            </button>
                        </>
                    )
