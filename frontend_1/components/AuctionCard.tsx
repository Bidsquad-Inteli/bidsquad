...
export const AuctionCard = ({ id, name, description, startPrice, price, startDate, endDate, address }: {
    id: string;
    name: string;
    description: string;
    startPrice: number;
        ...
        <Link href={`/auction/${id}`} className="flex flex-col w-[400px] border-[0.25px] border-grey-100 p-4 rounded-xl overflow-hidden hover:scale-105 transition duration-150 bg-white ease-in-out hover:cursor-grabbing	">
            <div className="flex flex-col w-full">
                ...
                <div className="w-full flex flex-row justify-start items-center gap-2 text-ellipsis ml-2 mt-4">
                    ...
                    <label className="text-ellipsis w-[80%]">{address.substring(0, 6)}...{address.substring(address.length - 4, address.length)}</label>
                </div>
                ...
                <div className="w-full flex flex-row justify-between items-center ml-2 mt-8">
                    <div className="flex flex-col items-start justify-center">
                        <label className="text-sm text-grey-200 font-bold">R$ {startPrice}</label>
                        <label className="text-2xl font-bold">R$ {price}</label>
                    ...
                    <div className="flex items-start justify-center mr-4">
                        <Link href={`/auction/${id}`}
                            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Ver Mais</Link>
                    </div>
                </div>
            </div>
        </Link>
